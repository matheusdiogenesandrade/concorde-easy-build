...
static int
    receive_tspfile (CC_SFILE *s, char *name, int id);
...
int main (int ac, char **av)
{
    ...
    while (1) {
        ...
        rval = receive_tspfile (s, probname, id);
        ...
        sprintf (buf, "lkh %s_par.%d", probname, id);
        p = popen (buf, "r");
        if (!p) {
            perror (buf);
...
static int receive_tspfile (CC_SFILE *s, char *name, int id)
{
    int rval = 0;
    int i, norm, ncount;
