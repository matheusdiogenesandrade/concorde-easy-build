        ...
        CCrandstate *rstate)
{
    ...
    if (ncount < 10 && nkicks > 0) {
        CC_PRINTF("Less than 10 nodes, setting nkicks to 0\n");  
        CC_FFLUSH(stdout);
        nkicks = 0;
    }
    ...
    *val = path_length (&G, ncount, tcyc, &D);
    ...
    if (outpath) {
        ...
        if (tval != *val) {
            CC_FPRINTF(stderr, "ERROR: Arrange path has wrong length\n");
            rval = 1;  goto CLEANUP;
        }
    }
...
static int load_path (graph *G, int *path, int *outpath)
{
    ...
    if (k != ncount) {
        CC_FPRINTF(stderr, "Did not gather the path correctly\n");
        return 1;
    }
