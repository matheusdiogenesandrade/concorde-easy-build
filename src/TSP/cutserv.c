...
int main (int ac, char **av)
{
    ...
    do {
        ...
        rval = serve_request (f, nodecount, pool, &status, numthreads,
                              &rstate);
        if (rval) {
            CC_FPRINTF(stderr, "serve_request failed\n");
            if (CCutil_sclose (f)) {
    ...
    *status = STATUS_NORMAL;
    ...
    switch (request) {
      case CCtsp_POOL_GETCUTS:
        ...
        if (rval) {
            CC_FPRINTF(stderr, "find_cuts failed\n");
            return rval;
        }
        break;
      case CCtsp_POOL_PUTCUTS:
        ...
        if (rval) {
            CC_FPRINTF(stderr, "receive_cuts failed\n");
            return rval;
        }
        break;
        ...
        int nthreads, CCrandstate *rstate)
{
    ...
    CC_PRINTF("Price "); CC_FFLUSH(stdout);
    ...
    if (ncount != ncount2) {
        CC_FPRINTF(stderr, "Request for wrong number of nodes (%d != %d)\n",
                 ncount, ncount2);
        rval = 1; goto CLEANUP;
    }
    ...
    if (elist == (int *)    NULL ||
        x     == (double *) NULL) {
        CC_FPRINTF(stderr, "Out of memory in find_cuts\n");
        rval = 1; goto CLEANUP;
    }
    ...
    CC_PRINTF("%d edges...", ecount); CC_FFLUSH(stdout);
    ...
    CC_PRINTF("%3d cuts (max %.3f)...", cutcount, maxviol); CC_FFLUSH(stdout);
    ...
    for (i=0; i<cutcount; i++) {
        ...
        if (rval) {
            CC_FPRINTF(stderr, "CCtsp_write_lpcut_in failed\n");
            goto CLEANUP;
        }
        cuts_next = cuts->next;
    ...
    f = (CC_SFILE *) NULL;
    if (rval) {
        CC_FPRINTF(stderr, "CCutil_sflush failed\n");
        goto CLEANUP;
    }

    CC_PRINTF("sent in %.2f (%.2f real)\n", CCutil_zeit() - szeit,
...
static int receive_cuts (CC_SFILE *f, int ncount, CCtsp_lpcuts *pool)
{
    ...
    CC_PRINTF("Cuts  "); CC_FFLUSH(stdout);
    rval = CCutil_sread_int (f, &ncount2);
    if (rval) {
        CC_FPRINTF(stderr, "CCutil_sread_int failed\n");
        goto CLEANUP;
    ...
    CC_PRINTF("%3d...", cutcount); CC_FFLUSH(stdout);

    savecount = pool->cutcount;
    for (i=0; i<cutcount; i++) {
        rval = CCtsp_read_lpcut_in (f, &newc, ncount);
        if (rval) {
            CC_FPRINTF(stderr, "CCtsp_read_lpcut_in failed\n");
            goto CLEANUP;
        }
    ...
    CC_PRINTF("%3d new in %.2f (%.2f real)  TOTAL %d\n", 
...
{
    CC_FPRINTF(stderr, "Usage: %s [-see below-] poolname\n", f);
    CC_FPRINTF(stderr, "  -n n # of nodes\n");
    CC_FPRINTF(stderr, "  -p n use port n\n");
    CC_FPRINTF(stderr, "  -P f initial pool file\n");
    CC_FPRINTF(stderr, "  -s n use random seed n\n");
    CC_FPRINTF(stderr, "  -T n use n threads\n");
    CC_FPRINTF(stderr, "   At least one of -n and -P must be provided\n");
}
...
int main (int ac, char **av)
{
    CC_FPRINTF(stderr, "Networking code not enabled - not able to serve cuts\n");
    return -1;
}
